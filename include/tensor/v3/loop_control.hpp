...
template <containers::concepts::StaticShape S, auto Start, auto End, auto Stride>
class loop_control
{
public:
    using shape_t = S;
            ...
            else
            {
                ...
                for (auto i = rank_t{}; i != s_rank; ++i)
                    start[i] = static_cast<index_t>(
                        static_cast<decltype(Start)>(s_sizes[i]) + Start + 1
                    );
                return start;
            }
        }
        ...
        if constexpr (std::is_arithmetic_v<end_t>)
        {
            ...
            else
            {
                ...
                for (auto i = rank_t{}; i != s_rank; ++i)
                        ...
                        static_cast<decltype(End)>(s_sizes[i]) + End + 1
                    );
                return end;
            }
        }
