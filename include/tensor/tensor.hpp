...
struct dynamic_tensor
{
    ...
    explicit dynamic_tensor(std::ranges::contiguous_range auto const& dims) noexcept
        ...
        , strides_{ sizes_ }
    {
        flat_size_          = sizes_[rank_ - 1];
        strides_[rank_ - 1] = size_type{ 1 };
        for (size_type i = rank_ - 1; i-- > size_type{};)
        {
            strides_[i] = strides_[i + 1] * sizes_[i + 1];
            flat_size_ *= sizes_[i];
        }
        buffer_ = std::vector(flat_size_, value_type{});
    }
        ...
        -> decltype(auto)
    {
        ...
        auto const i = std::inner_product(
            std::cbegin(idxs), std::cend(idxs), std::begin(self.strides_), index_t{}
        );
    ...
    [[nodiscard]]
    auto size(const index_t i) const noexcept -> size_type
    {
        assert(i < rank_);
        return sizes_[i];
        ;
...
inline std::vector<size_t> compute_output_shape(
    dynamic_tensor<T> const&                           a,
    dynamic_tensor<T> const&                           b,
    std::vector<std::pair<
        typename dynamic_tensor<T>::size_type,
        typename dynamic_tensor<T>::size_type>> const& cis
)
{
    using tensor_t           = dynamic_tensor<T>;
...
auto contract(
    ...
    dynamic_tensor<T> const&                         B,
    std::vector<std::pair<
        typename dynamic_tensor<T>::index_t,
        typename dynamic_tensor<T>::index_t>> const& cis
)
{
    using size_type = typename dynamic_tensor<T>::size_type;
    using index_t   = typename dynamic_tensor<T>::index_t;
...
template <typename T>
auto operator<<(std::ostream& os, dynamic_tensor<T> t) noexcept -> std::ostream&
{
    multi_index midx(t.sizes_, t.strides_, t.rank());
    do
    {
        std::cout << t[midx] << ", ";
